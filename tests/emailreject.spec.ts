...
async function waitForNextApproval(page: Page, maxAttempts: number = 10, interval: number = 3000): Promise<boolean> {
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (emailInfo) {
            ...
            approvalCount++; // 增加审批计数
            console.log('当前是第', approvalCount, '步审批');
            
            // 如果是第四步审批，使用拒绝链接
            const url = approvalCount === 4 ? emailInfo.rejectUrl : emailInfo.approveUrl;
            await page.goto(url);
            ...
            //         await page.fill(commentSelector, '第四步拒绝：数量不合适');
            //     }
            // }
            ...
            // 验证操作成功
            if (approvalCount === 4) {
                // 拒绝操作的成功信息可能不同，需要根据实际情况调整
                // await expect(page.locator('.messages .success')).toBeVisible();
                await expect(page.locator('h1')).toHaveText('审批已拒绝');
                await expect(page.locator('p')).toHaveText('您的审批请求已被拒绝。');
            } else {
        ...
        while (hasMoreApprovals) {
            ...
            if (!hasMoreApprovals) {
                ...
                if (finalStatus) {
                    ...
                    console.log('申请被拒绝，拒绝详情:', finalStatus.details);
                }
                console.log('审批流程完成');
                break;
            }
